...
Class cWDInclEO135
	...
	'Declarations
	Private m_oAIRStats(6)		' AIRStatistics
	...
	Public Default Function construct( gtar )
		...
		Set m_oAIRStats(1) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI017) )
		Set m_oAIRStats(2) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI018) )
		Set m_oAIRStats(3) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI019) )
		Set m_oAIRStats(4) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI020) )
		Set m_oAIRStats(5) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI022) )
		Set m_oAIRStats(6) = (new cAIRStatistics)( Array("135", m_strGTAR, TSOL_TXCODI009) )
		...
		m_oAIRStats(5).GetValues(dt)
		m_oAIRStats(6).GetValues(dt)
		Calculate()
        Set construct = Me
    End Function
  ...
  <script type="text/javascript">
	google.load('visualization', '1', { packages: ['corechart'] });
   	google.setOnLoadCallback(drawChart<%=m_sTitle %>);
		...
		data.addColumn('number', 'CLI Delayed');
...
<%
		...
		m_oAIRStats(5).oData.Open()
		m_oAIRStats(6).oData.Open()
		If m_oAIRStats(1).oData.FetchStart() > 0 And m_oAIRStats(2).oData.FetchStart() > 0 And _
		    m_oAIRStats(3).oData.FetchStart() > 0 And m_oAIRStats(4).oData.FetchStart() > 0 And _
		     m_oAIRStats(5).oData.FetchStart() > 0 And m_oAIRStats(6).oData.FetchStart() > 0 Then
			Do
				...
				Do While m_nDate < m_oAIRStats(4).oData.dtDate
					...
					End If
				Loop
				Do While m_nDate < m_oAIRStats(5).oData.dtDate
					If m_oAIRStats(4).oData.FetchNext() < 1 Then
						Exit Do
					End If
				Loop
				Do While m_nDate < m_oAIRStats(6).oData.dtDate
					If m_oAIRStats(5).oData.FetchNext() < 1 Then
						Exit Do
					End If
				Loop
				If m_nDate = m_oAIRStats(6).oData.dtDate And _
			...
			End If
			Loop While m_oAIRStats(1).oData.FetchNext() > 0 And _
						m_oAIRStats(2).oData.FetchNext() > 0 And _
						 m_oAIRStats(3).oData.FetchNext() > 0 And _
						  m_oAIRStats(4).oData.FetchNext() > 0 And _
						   m_oAIRStats(5).oData.FetchNext() > 0 And _
						    m_oAIRStats(6).oData.FetchNext() > 0
			m_oAIRStats(1).oData.Close()
			m_oAIRStats(2).oData.Close()
			m_oAIRStats(3).oData.Close()
			m_oAIRStats(4).oData.Close()
			m_oAIRStats(5).oData.Close()
			m_oAIRStats(6).oData.Close()
		End If
		 %>
				]);
			var options = {
  ...
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
        ...
        <td colspan=4 align="left"><a href="/AvGeral/CtrlProcMntRelatPendBySolic.asp?Solic=<%=TSOL_TXCODI017 %>&SDiv=<%=m_strGTAR %>&Sec=135"
		 style="text-decoration:none; color:Black;" target="_blank"><font
		 size="5" face="Calibri"><strong>Certificação/Inclusão na EO de Táxi Aéreo - RBAC 135</strong></font></a></td>
	    <td width=26 height=30><img
  ...
  <table border="0" width="100%">
	...
	Private Sub GetValues()
		...
		m_oAIRData.Add(m_oAIRStats(5).oData)
		m_oAIRData.Add(m_oAIRStats(6).oData)
	End Sub
