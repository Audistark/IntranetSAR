	...
	Public Function GetValues()
		...
		Do
			...
			If ret > 0 Then
				...
				' Se não é hoje não precisa recalcular
				...
				Exit Do ' força saida (jump)

			Else ' not found

				' Se não existe porém é sábado ou domingo...
				Dim wday : wday = Weekday(dtToday) ' sábado e domingo não
				If wday = vbSunday Or wday = vbSaturday Then
					If try < 0 Then
						try = 5 ' tenta voltar até 5 dias
					ElseIf try = 0 Then
						GetValues = -1 ' não encontrou registro
						Exit Function
					Else
						try = try - 1
					End If
