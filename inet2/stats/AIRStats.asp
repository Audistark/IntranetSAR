...
<head>
  ...
  <script type="text/javascript">
	...
	function checkFiles() {<%
		...
		document.getElementById("IconSendMail").title = "Email já enviado";
	...
	function sendmail() {
		if (!confirm('Deseja enviar e-mail das estatísticas aos gestores?\n\n')) {
			return;
		}
		else {
...
<body>
		...
		<td align="center" valign="bottom"><table border="0" cellpadding="1" cellspacing="0">
			<tr>
				...
				If optSendMail = True Then
					...
					<img src="../img/icons/glyphicons_128_message_lock.png" width="28" height="24" border="0" id="IconSendMail" title="Email já enviado"><%
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="98%">
	...
	Observações:<br>
	1. Atrasos da ANAC são computados a partir da meta estabelecida ou depois de 60 dias se não houver meta para o grupo estatístico.<br>
	2. Atrasos do Cliente são computados a partir do prazo estabelecido pela ANAC + uma tolerância de 30 dias;<br>
	3. Os cursores dos gauges indicam a posição dos processos em relação aos atrasos, sejam da ANAC ou do Cliente;<br>
	4. O 'gráfico de tempos em dias' é mostrado apenas quando há meta estabelecida para o grupo estatístico;<br>
	5. O alarme dispara se houver algum processo que não esteja atendendo a meta estabelecida;<br>
	6. Os '30d' Docs e Closed mostram o número de documentos emitidos e processos concluídos pela ANAC nos últimso 30 dias;<br>
	  </font></td>
	</tr>
  </table>
...
Else ' Send mail (step 2)
	...
	Dim mailSubject : mailSubject = "[SAR-GGAC/GCVC] Estatísticas dos processos RBAC " & rbac & " prioritários da GGAC"
	Dim hs : hs = Hour(Now())
	Dim Greetings
	If hs < 13 Then
		Greetings = "bom dia"
	...
	Dim mailBody : mailBody = "<html>" & vbCrLf & _
		...
		"<p><font size=""3"" face=""Calibri"">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;A seguir as estatísticas atualizadas dos processos prioritários da GGAC: </font></p>" & vbCrLf
	...
	mailBody = mailBody & "<br>" & vbCrLf & _
			...
			"	Observações:<br>" & vbCrLf & _
			"	1. Atrasos da ANAC são computados a partir da meta estabelecida.<br>" & vbCrLf & _
			"	2. Atrasos do Cliente são computados a partir do prazo estabelecido pela ANAC + uma tolerância de 30 dias;<br>" & vbCrLf & _
			"	3. Os '30d' Docs e Closed mostram o número de documentos emitidos e processos concluídos pela ANAC nos últimos 30 dias.<br>" & vbCrLf & _
			"   Maiores informações acerca dos dados estatísticos podem ser obtidas em http://sar/inet2/stats/AIRStats.asp<br><br>" & vbCrLf & _
			"	Atenciosamente,<br>" & vbCrLf & _
			"	Gerência de Coordenação da Vigilância Continuada - GCVC<br>" & vbCrLf & _
			"	  </font></td>" & vbCrLf & _
			"	</tr>" & vbCrLf & _
			"  </table>" & vbCrLf & _
			"</html>" & vbCrLf
